--- title: "Integrative visualization of TCGA public data with
R/Bioconductor Packages" author: "Hongen Zhang and Sean Davis" output:
html_document bibliography: citations.bib ---
...
to them. We simply use Bioconductor biomaRt package [21, 22] to get
...
and limma pacakges [23-26] on TCGA LIHC RNAseq data. The scripts used
for normalization [27, 28] and PCA plot [29, 30] are included in
supplement file 3 and 4.
...
polygon, points, lines or link line plots [31]. CIRCOS plot can
display multi-type of genomic alterations for whole or part of genome
but only limited samples and limited genes can be presented. There are
several implementations for CIRCOS plot in R or Bioconductor packages
[14, 32-34]. In this manuscript, we are focusing on RCirocs package
only.
...
1. 1)Genomic coordinate information with alteration measurements
   ...
   gene labeling.
2. 2)Single genomic coordinate information only in each row. This type
   of data is usually plotted as tiles or points for genomic segments
   without measurement values such as mutation points.
3. 3)Paired genomic coordinate information in each row. The linkages
...
1. 1)Prepare data in correct forms required by RCircos: This step
...
2. 2)Initialize core components by call RCircos.Set.Core.Components()
...
functionality [30, 35-36], ComplexHeatmap provides a highly flexible
